import { Filter, Plus, RotateCcw, Star, X } from 'lucide-react'
...
interface MapSidebarProps {
  ...
  addMode: boolean
  hideSearch?: boolean
  listMode?: 'all' | 'hidden-camping' | 'hidden-cafe' | 'favorites'
  favoriteIds?: string[]
  isFavorite?: (id: string) => boolean
  onToggleFavorite?: (id: string) => void
  onClearListMode?: () => void
...
export function MapSidebar({
  ...
  const listModeLabel =
    listMode === 'hidden-camping'
      ? 'Camping jarang dikunjungi'
      : listMode === 'hidden-cafe'
        ? 'Cafe jarang dikunjungi'
        : listMode === 'favorites'
          ? 'Lokasi favorit'
          : null
  ...
  return (
    <aside className="flex h-full w-full flex-col border-r border-slate-200 bg-white text-slate-900 lg:w-80 xl:w-96 shrink-0">
      ...
      {listModeLabel && onClearListMode && (
        <div className="flex items-center justify-between gap-2 border-b border-cyan-100 bg-cyan-50 px-4 py-2">
          <p className="text-xs font-semibold text-cyan-900">{listModeLabel}</p>
          <button
            type="button"
            onClick={onClearListMode}
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  {onToggleFavorite && isFavorite && (
                    ...
                    >
                      <Star
                        className={`h-4 w-4 ${isFavorite(pin.id) ? 'fill-amber-400 text-amber-500' : 'text-slate-300'}`}
                      />
                    </button>
                  )}
                </div>
